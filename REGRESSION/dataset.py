...
class listDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train == True:
            ...
            img, label_index = self.load_valid_data_label(imgpath)
            # img   = torch.from_numpy(img).float()
            label_index = torch.LongTensor([label_index])
...
#         exposure = 0
        ...
        label_index = int(label_path) #REGRESSION
...
#         classes = []
#         for line in open('/timo/datasets/Dua/GRU-test/Data-Combinations/Classification/classInd.txt'):
